...
void Ray_Tracer::ray_tracing(M3DVector3f start, M3DVector3f direct, float weight, M3DVector3f color, unsigned int depth, Basic_Primitive * prim_in)
{
	...
	else
	{
		...
		if (type != _k_miss)
		{
			//set up shading to get local color
			...
			//get properties ks kt ws wt
			prim->get_properties(ks, kt, ws, wt);
		
			//get reflect direction
			prim->get_reflect_direct(direct, intersect_p, reflect_direct);
			//get reflect color by recusrively calling ray_tracing (Is)
			ray_tracing(intersect_p, reflect_direct, weight * ws, reflect_color, depth, prim_in);

			//get delta value
			float delta = prim->get_delta();
			//get refract direction
			prim->get_refract_direct(direct, intersect_p, refract_direct, delta, type);
			//get refract color by recursively calling ray_tracing (It)
